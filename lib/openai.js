...
export async function callAlex(env, level, playerPrompt) {
  const data = await postChatCompletion(env.OPENAI_API_KEY, {
    model: ALEX_MODEL,
    // "none" -- reine Persona-Textausgabe braucht kein Reasoning, spart spuerbar Latenz
    // (wichtig fuer "sofortiges Feedback", siehe PLAN Abschnitt 1/Kontext).
...
export async function callJudge(env, level, alexResponse) {
  ...
  const data = await postChatCompletion(env.OPENAI_API_KEY, {
    model: JUDGE_MODEL,
    // "low" -- Rubrik-Abgleich ist eine einfache Klassifikationsaufgabe, kein tiefes
    // Reasoning noetig; etwas mehr als "none" fuer Zuverlaessigkeit bei der Bewertung.
