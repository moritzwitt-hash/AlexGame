// Handler für POST /api/attempt -- Haupt-Endpoint (siehe PLAN Abschnitt 1/5).
// Orchestriert Ausführungs- und Judge-Call in einem Roundtrip. Der Client
...
import { callAlex, callJudge, OpenAICallError } from "./openai.js";

const MAX_PROMPT_LENGTH = 4000;

// Hint-Zusammenbau (PLAN Abschnitt 5b): drei Eskalationsstufen.
// Versuch 1 = sanfter Wink, Versuch 2 = expliziter CARE-Buchstabe,
// ab Versuch 3 = konkretes Beispiel-Snippet als Starthilfe (vorgezogen von
// Versuch 4, damit niemand bei der knappen Session-Zeit zu lange festhängt).
function buildHint(level, attemptNo, judgeVerdict) {
  ...
  if (attemptNo === 2) return level.hints.afterAttempt2;
...
export async function handleAttempt(request, env) {
  let body;
  try {
    body = await request.json();
  } catch {
    return jsonResponse({ error: "Ungültiger Request-Body (kein JSON)." }, 400);
  }
